...
begin
 ...
 try
  ...
  if Length(ACaption) = 0 then
   begin
    if ASub = nil then Caption:=GetCaption
    else Caption:=ASub.Caption;
   end;
      ...
      begin
       ...
       if Current.Handle = Handle then
        begin
         ...
         if WorkTime <> 0 then AddItem(AResponse,'CreateTime:',DateTimeToStr(WorkTime))
         else AddItem(AResponse,'CreateTime:','N/A');

         WorkTime:=SystemFileTimeToDateTime(TFileTime(Current.ExitTime));
         if WorkTime <> 0 then AddItem(AResponse,'ExitTime:',DateTimeToStr(WorkTime))
         else AddItem(AResponse,'ExitTime:','N/A');
